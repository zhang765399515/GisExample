...
export function loadGltf(){
    window.viewer.camera.flyTo({
        destination:Cartesian3.fromDegrees(126.392,45.912,10)
    })
    const model = window.viewer.entities.add({
        position: Cartesian3.fromDegrees(126.392,45.912,0),
        model: {
          scale: 0.01,
...
export class ModelEditHelper {
    constructor( model, opt) {
        ...
        this.opt = opt || {};
        this.model = model;
        this.center = this.model.position._value;
        this.handler = undefined;
        this.helperModel = this.createHelperModel(this.center, (this.opt.helperModel.scale || 10)); // 辅助模型
        this.helperModel.show = true; // 默认初始化时 不展示模型  start开始展示
        this.axisType = undefined; // 标识当前操作的轴对象
        this.startPX = undefined; // 起始点坐
    ...
    bindHandler() {
        ...
        this.handler.setInputAction(evt => {
            ...
            if (this.axisType.indexOf('Axis') != -1) { // 旋转
                ...
                this.setAxisPosition({position:position,heading:this.heading,pitch:this.pitch,roll:this.roll,scale:this.scale});
    ...
    start() {
        if (this.helperModel) {
            ...
            this.model.position.setValue(this.center.clone());
    ...
    setAxisHpr(heading, pitch, roll) {
        ...
        this.model.orientation = Transforms.headingPitchRollQuaternion(
            this.helperModel.position.getValue(viewer.clock.currentTime), new HeadingPitchRoll(
                CMath.toRadians(heading || 0),
                CMath.toRadians(pitch || 0),
                CMath.toRadians(roll || 0)
