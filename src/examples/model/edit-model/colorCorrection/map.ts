...
export async function load3dtiles() {
    ...
    const screenLeft = window.viewer.scene.primitives.add(tilesetRight);
    console.log('name：screenLeft', screenLeft);
    // screenLeft.readyPromise.then(til => {
        window.viewer.zoomTo(screenLeft)
        let ent = window.viewer.entities.add({
            position: screenLeft.boundingSphere.center,
            model: {
        ...
        //     fragmentShader: fs,
        //     uniforms: {
        //         colorTexture: new Color(1.0, 0.0, 0.0, 1.0),
        //         highlight: function () {
