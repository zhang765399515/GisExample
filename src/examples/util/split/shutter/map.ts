import { Geokey3DTileset, SplitDirection, ScreenSpaceEventHandler, ScreenSpaceEventType } from "geokey-gis";
...
export async function splitShutter() {
  ...
  const tilesetRight = await Geokey3DTileset.fromUrl('http://14.22.86.227:12022/service/gis/3DModel/?serviceName=xms_b3dm_bk')
  const screenLeft = window.viewer.scene.primitives.add(tilesetLeft);
  const screenRight = window.viewer.scene.primitives.add(
    tilesetRight
  );
