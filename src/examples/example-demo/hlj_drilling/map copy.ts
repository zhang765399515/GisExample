...
export async function loadDrill() {
    ...
    for (const key in drillData) {
        ...
        drillListData.forEach((e, index) => {
            ...
            if (index == 0 || index == drillListData.length - 1) {
                ...
                drawCircular('circular', positions, height, e.TCMC, new Cartesian2(-2, 2))
            }
    ...
    for (const e of drillGroup) {
        ...
        let img_h = -3.14 * 2
        var sideAppearance = await createAppearence(e[0].TCMC, new Cartesian2(img_h, e[0].TCHD), h == 0 ? 1 : h);
...
export async function load3dtiles() {
    ...
    try {
        const tileset = await Geokey3DTileset.fromUrl('http://14.22.86.227:12022/service/gis/3DModel/HRF010/tileset.json?serviceName=HRF010_P1_b3dm');
 ...
 * @param repeat 重复次数
 */
export function drawCircular(name, position, height, tcmc, repeat) {
    const circular = new Primitive({
        ...
        appearance: createAppearence(tcmc, repeat, height)
    });
...
function createAppearence(tcmc, repeat, height) {
    let fs = `czm_material czm_getMaterial(czm_materialInput materialInput)\
      {
                ...
                // float t0 = floor(2.0*(depth - startDistance) / (endDistance - startDistance)); \
                ...
                vec3 normalEC = materialInput.normalEC;
                vec4 color = vec4(0.0);
                color = texture2D(image, fract(vec2(repeat.x,repeat.y) * materialInput.st)).rgba;
                material.diffuse=color.rgb;
                material.normal = -normalEC;
                material.alpha = 1.0;
                return material;
            }`;
    let center = Cartesian3.fromDegrees(125.902601, 47.190629, 50)
    const _Cartesian4Center = new Cartesian4(center.x, center.y, center.z, 1);
    const repeatY = Math.ceil(height / 200);
        ...
        material: new Material({
            fabric: {
                ...
                uniforms: {
                    ...
                    // fogByDistance: new Cartesian4(10, 0.0, 5000, 1.0),
                },
                // source: fs
            }
        }),
...
export async function drawOnePolygon(type, val: any[]) {
    let positions: any[] = [], colorData: any[] = [], polygonPositions: any[] = []
    // for (const key in drillData) {
    //     positions.push(drillData[key].WGSX, drillData[key].WGSY)
    // }
    let distance = 0, maxDistance = 0, minDistance = 0;
    val.forEach((e, index) => {
        if (index <= val.length - 2) {
            let lastPoint = Cartesian3.fromDegrees(val[index + 1][0], val[index + 1][1])
            let firstPoint = Cartesian3.fromDegrees(e[0], e[1])
            ...
            distance = distance + Cartesian3.distance(firstPoint, lastPoint)
        }
        positions.push(e[0], e[1], 0)
        colorData.push(Color.fromRandom({ alpha: 1 }))
    })
    positions.push(val[0][0], val[0][1], 0)
    for (const key in drillData) {
        const XQ = drillData[key].XQ;
        let pointData: any[] = [], polygonData = {
            "heightData": [],
            "colorData": []
        };
        val.forEach(e => {
            pointData.push(e[0], e[1], XQ[0].CDGC)
        })
        for (const [index, child] of XQ.entries()) {
            let instances: GeometryInstance[] = [], heightData: number[] = [];
            var height = child.CDGC / 1 + 5;
            var extrudedHeight = (child.CDGC + child.TCHD) / 1 + 5;
            heightData.push(child.TCHD * 1)
            if (index == XQ.length - 1) {
                polygonData.heightData.push(child.CDGC);
            } else {
                polygonData.heightData.push(child.CDGC + child.TCHD);
            }
            polygonData.colorData.push(colorsData[child.TCMC]);
            child.WGSX = drillData[key].WGSX;
            child.WGSY = drillData[key].WGSY;
            child.ID = key;

            if (index == 0 || index == XQ.length - 1) {
                let height = index == 0 ? (child.CDGC + child.TCHD + 5).toFixed(2) : (child.CDGC + 5).toFixed(2)
                if (type == 1) {
                    drawCircular('drillPictures', positions, height, child.TCMC, new Cartesian2(-maxDistance * 0.25, minDistance * 0.25))
                } else {
                    loadPolygon('drillPictures', positions, height, colorsData[child.TCMC])
                }
            }
            var sideGeometry = createWallGeometry(positions, height, extrudedHeight);
            var sideGeometryInstance = new GeometryInstance({
            ...
            let h = Math.max(...heightData) - Math.min(...heightData)
            if (type == 1) {
                var sideAppearance = await createAppearence(child.TCMC, new Cartesian2(-distance * 0.5, child.TCHD * 0.5), h == 0 ? 1 : h);
                var sidePrimitive = new Primitive({
                    geometryInstances: instances,
                    appearance: sideAppearance
                });
                let primitive = window.viewer.scene.primitives.add(sidePrimitive);
                primitive.name = 'drillPictures'
            } else {
                drawPolygon('drillPictures', pointData, polygonData)
            }
        }
    }
}
...
export function moveListener(point, data, distance) {
    ...
    handler.setInputAction((event) => {
        ...
        console.log('name：scale', height, scale);
        if (scale <= 8) {
