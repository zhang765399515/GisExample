...
export function loadRegions(data) {
    ...
    wallMaterial.uniforms.color = Color.fromCssColorString("rgb(0,79,168)");
    ...
    wallPrimitive.name = 'xzqh'
    window.viewer.scene.primitives.add(wallPrimitive);
}

export function loadLine(data) {
    ...
    const linesData = GeoJsonDataSource.load(data);
    linesData.then(function (dataSource: any) {
        dataSource.name = 'xzqhLine'
        let myLine = dataSource.entities.values;
        myLine.forEach((entity: Entity) => {
            // entity.polyline.material = Color.fromCssColorString("#0000ff")
...
export function loadDynamicLine2(data, color, type) {
    const geometryInstance = new GeometryInstance({
        geometry: new PolylineGeometry({
    ...
    const material = new Material({
        ...
        fabric: {
            uniforms: {
                ...
                image: `image/ArrowOpacity/${type}.png`,
                speed: 2,
                imageW:500,
                imageh:1000
            },
        ...
        czm_material czm_getMaterial(czm_materialInput materialInput) {
          czm_material material = czm_getDefaultMaterial(materialInput);
          vec2 st = materialInput.st;
          float pixelRatio = czm_pixelRatio;
          float st_map_s = fract(st.s - speed * czm_frameNumber * 0.001);
          vec4 colorImage = texture2D(image, vec2(st_map_s, st.t));
          vec4 fragColor;
    ...
    primitive.name = 'arrowOpacity' + type;
    window.viewer.scene.primitives.add(primitive)
}
